...
export const fillTemplate = async (templateUrl, request) => {
  try {
    ...
    drawText(`${request.client.contract_number}`, 650, height - 180);
  ...
  drawText(`${parseFloat(Number(operation.quantity)).toFixed(2)} ${await fetchSolutionUnitOfMeasure(operation.solutionId)}`, quantityXPosition, yPosition);
    ...
    drawText(`${request.observations}`, 190, 1500); // Adjusted Y position for observations
    ...
    if (request.apparateSgr) {
      drawText('10. S-a efectuat operatiunea de dezinsectie + deeatizare container SGR', 100, height - 465, rgb(0, 0, 0), 10);
    }
